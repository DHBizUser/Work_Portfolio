...
cd "S:\filepath\to\the\commit\repo"
git pull
...
.mode tabs
.import "source-BOM-data.txt" BOMreportimport
/*==BODY=====================================================================*/

CREATE TABLE IF NOT EXISTS Parms (_key, _value);
INSERT INTO Parms VALUES
	('iter',1),('FocusCust','--'),('ReportDate',(SELECT _1 FROM BOMreportimport LIMIT 1)),
	('FocusBOMstatus','01');

CREATE TABLE IF NOT EXISTS UnitEqivalences (_key, _value);
...
CREATE TABLE IF NOT EXISTS BOMPairstable AS
	SELECT DISTINCT
		NULL AS BOMdepth,
		_6 AS Cust,
		_4 AS ParentMtrl,
		...
		SUBSTR('   '||_11,-3,3) AS ParentUnit,
		_15 AS ComponentMtrl,
		_16 AS ComponentDesc,
		NULLIF(REPLACE(TRIM(_18),',','')*1.0,"") AS ComponentQty,
		IIF(_19 = "******"," GL",
			SUBSTR('   '||_19,-3,3)) AS ComponentUnit,
		NULLIF(REPLACE(TRIM(_18),',','')*1.0,"")
			/NULLIF(REPLACE(TRIM(_9),',','')*1.0,"") AS UnitUsage,
		IIF(_19 = '******',' GL',SUBSTR('   '||_19,-3,3)) || '*' ||
			SUBSTR('   '||_11,-3,3) || '^-1'
			AS ConversionUnit,
		_12 AS AltBOM,	
		_2 AS BOM,
	...
	WHERE _2 LIKE '00%' AND LENGTH(_15) > 0;


CREATE TABLE IF NOT EXISTS BOMCompData AS SELECT
	_15 AS Material,
	_16 AS Description,
	IIF(_32 = 'S',NULLIF(TRIM(_33)*1.0,"")
		/ NULLIF(REPLACE(TRIM(_35),',','')*1.0,""),
		IIF(_32 = 'V',NULLIF(TRIM(_34)*1.0,"")
			/ NULLIF(REPLACE(TRIM(_35),',','')*1.0,""),NULL))
			AS UnitPrice,
	IIF(_19 = "******","GL",_19) AS ComponentUnit,
	IIF(TRIM(_23)>0,TRIM(_23),NULL)
	AS LeadTime
	FROM BOMReportimport WHERE _2 LIKE '00%' AND LENGTH(_15) > 0;


CREATE VIEW IF NOT EXISTS TopLvlParent AS
...
CREATE VIEW IF NOT EXISTS _tblGenerateSQL AS SELECT
'CREATE TABLE ' || _tbl || ' AS SELECT * FROM _G00TLpaths;
UPDATE ' || _tbl ||
	' SET
		EqHandler =
...
/* SQLite has a limit of 500 clauses bound by UNION ALL into the same
statement, so I made 2 intermediate tables and then combined those in
another step
=====================================================================*/

CREATE TABLE IF NOT EXISTS _PathsSQL01 AS SELECT
'CREATE TABLE IF NOT EXISTS _01Paths AS' AS "--_PathsSQL01"
Union ALL SELECT
...
'SELECT * FROM ' ||
	...
	|| ';  CREATE TABLE IF NOT EXISTS _02Paths AS'
WHERE rowid = (SELECT MAX(rowid)FROM _PathsSQL01);


CREATE TABLE IF NOT EXISTS _PathsSQL02 AS SELECT
...
CREATE VIEW IF NOT EXISTS _AllPathsSQL AS
	SELECT * FROM _PathsSQL01 UNION ALL
	SELECT * FROM _PathsSQL02;


CREATE VIEW IF NOT EXISTS _G00TLpaths AS
